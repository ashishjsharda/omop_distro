...
CREATE TABLE IF NOT EXISTS results.cohort_features
...
CREATE TABLE IF NOT EXISTS results.cohort_features_analysis_ref
 (cohort_definition_id bigint,
  analysis_id int, 
...
CREATE TABLE IF NOT EXISTS results.cohort_features_dist
 (cohort_definition_id bigint,   
  covariate_id bigint, 
  count_value NUMERIC, 
  min_value NUMERIC, 
...
CREATE TABLE IF NOT EXISTS results.cohort_features_ref
 (cohort_definition_id bigint,
  covariate_id bigint, 
...
LEFT JOIN vocab.concept soc ON hlgt_to_soc.soc_concept_id = soc.concept_id;
...
LEFT JOIN vocab.concept_ancestor ca1 ON M.concept_id = ca1.DESCENDANT_CONCEPT_ID AND ca1.min_levels_of_separation = 1
LEFT JOIN vocab.concept c1 ON ca1.ANCESTOR_CONCEPT_ID = c1.concept_id
LEFT JOIN vocab.concept_ancestor ca2 ON c1.concept_id = ca2.DESCENDANT_CONCEPT_ID AND ca2.min_levels_of_separation = 1
LEFT JOIN vocab.concept c2 ON ca2.ANCESTOR_CONCEPT_ID = c2.concept_id
LEFT JOIN vocab.concept_ancestor ca3 ON c2.concept_id = ca3.DESCENDANT_CONCEPT_ID AND ca3.min_levels_of_separation = 1
LEFT JOIN vocab.concept c3 ON ca3.ANCESTOR_CONCEPT_ID = c3.concept_id
...
LEFT JOIN vocab.concept_ancestor ca1 ON obs.concept_id = ca1.DESCENDANT_CONCEPT_ID AND ca1.min_levels_of_separation = 1
LEFT JOIN vocab.concept c1 ON ca1.ANCESTOR_CONCEPT_ID = c1.concept_id
LEFT JOIN vocab.concept_ancestor ca2 ON c1.concept_id = ca2.DESCENDANT_CONCEPT_ID AND ca2.min_levels_of_separation = 1
LEFT JOIN vocab.concept c2 ON ca2.ANCESTOR_CONCEPT_ID = c2.concept_id
LEFT JOIN vocab.concept_ancestor ca3 ON c2.concept_id = ca3.DESCENDANT_CONCEPT_ID AND ca3.min_levels_of_separation = 1
...
(
	...
	FROM vocab.concept c1
	INNER JOIN vocab.vocabulary v1 ON c1.vocabulary_id = v1.vocabulary_id
...
LEFT JOIN (
	...
	FROM vocab.concept_ancestor ca0
	INNER JOIN (
		SELECT DISTINCT c2.concept_id AS os3_concept_id
		FROM vocab.concept_ancestor ca1
		INNER JOIN vocab.concept c1 ON ca1.DESCENDANT_CONCEPT_ID = c1.concept_id
		INNER JOIN vocab.concept_ancestor ca2 ON c1.concept_id = ca2.ANCESTOR_CONCEPT_ID
		INNER JOIN vocab.concept c2 ON ca2.DESCENDANT_CONCEPT_ID = c2.concept_id
